...
class FacetEfficientEvaluation:
    ...
    def get_facet_words(self, document: Document, facet_name: str, topic_dict, summary_dict):
        facet_words = set()
        ...
        # print(facet_words[:10], facet_name, len(facet_words))
        return facet_words
    ...
    def word_neighborhood_evaluation(self, facet_name: str, doc_id: str, doc_top_n: int, word_top_n: int,
        ...
        facet_words = self.get_facet_words(document, facet_name, topic_dict, summary_dict)
    ...
    def evaluate(self, topic_dict, summary_dict, doc_top_n: int = 20, word_top_n: int = 1000):
        ...
        for doc_id, document in tqdm(self.corpus.documents.items(),
            ...
            for facet_name in self.facets:
                ...
                score_relaxed, score_strict = self.word_neighborhood_evaluation(facet_name, doc_id, doc_top_n,
                                                                                word_top_n,
                                                                                is_series_corpus, document, topic_dict,
                                                                                summary_dict)
...
def calculate_facet_scores(data_sets: List[str], vector_names: List[str], facets: List[str]):
    ...
    for data_set in data_sets:
        ...
        df = df.sort_values(['Corpus', 'Facet', 'Algorithm', 'Relaxed Score', 'Strict Score'])
        print(df)
